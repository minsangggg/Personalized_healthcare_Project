...
def run():
    ...
    fig_usage.update_traces(marker=dict(color=THEME_COLORS["orange"], size=8))
    apply_theme(fig_usage)
    ...
    apply_theme(fig_daily_any:=fig_daily7)
    st.plotly_chart(fig_daily_any, width="stretch")
    ...
    today_list = qdf(
        f"""
        SELECT content_id, id, content_title, like_count, created_at
        FROM {T('board')}
        WHERE DATE(created_at) = CURDATE()
