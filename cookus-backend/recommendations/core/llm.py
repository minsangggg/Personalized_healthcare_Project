...
[이름 일관성 + 자연스러움 규칙]
- 최종 요리 이름(recipe_nm_ko)은 한국 음식 메뉴판에 있을 법한 자연스러운 한 줄짜리 이름이어야 합니다.
- 이름에는 **최대 1~2개의 핵심 재료명**만 넣고, 나머지 재료는 '채소', '야채', '모둠채소', '볶음밥', '덮밥'처럼 뭉뚱그려 표현하세요.
  - 예: '닭고기 채소볶음', '계란 프리타타', '닭고기 야채덮밥' 등
- 이름에 재료를 3개 이상 나열하거나, '·', ',', '/' 등으로 재료를 줄줄이 나열하지 마세요.
  - ❌ '닭고기·양파·당근 볶음' 같은 형태는 피합니다.
- 이름에는 재료 개수/용량/단위(1개, 1/2개, g, 컵 등)를 넣지 마세요.
- 원본 레시피 이름이 자연스럽고, ingredient_full에 포함된 핵심 재료와 모순되지 않으면 그대로 사용해도 좋습니다.
- 단, 원본 이름에 포함된 핵심 재료(예: 오징어, 미나리, 새우, 조개, 소고기, 돼지고기, 닭고기, 면, 밥, 빵 등)가
  최종 ingredient_full에 없다면 그 단어가 들어간 이름은 그대로 쓰지 말고,
  실제 사용하는 재료를 기준으로 새로운 이름을 지어 주세요.
  - 예: '오징어미나리초무침' → '채소 초무침', '짬뽕' → '얼큰 채소탕' 처럼.
